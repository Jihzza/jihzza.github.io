...
export default function ChatbotPage() {
  ...
  // Consume pending welcome message or generate initial greeting
  useEffect(() => {
    try {
      ...
      // Check if we need to show initial welcome message
      const hasShownWelcome = sessionStorage.getItem('chatbot-welcome-shown');
      if (!hasShownWelcome && isAuthenticated) {
        const welcomeMessage = `👋 Welcome! I'm here to help you with Daniel Dagalow's business coaching services. I can assist you with:

• **Scheduling consultations** (€90/hour)
• **Coaching subscriptions** (Basic, Standard, or Premium plans)
• **Pitch deck requests** (GalowClub & Perspectiv)
...
What can I help you with today?`;
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      // Regular message processing via OpenAI
      const result = await chatbotService.sendMessage(content, sessionId, user?.id);
      
      if (result.success) {
        setMessages((prev) => [...prev, { role: 'assistant', content: result.response }]);
      } else {
        console.error('Chatbot Service Error:', result.error);
        setMessages((prev) => [...prev, { role: 'assistant', content: result.response }]);
      }
    } catch (err) {
      setMessages((prev) => [
