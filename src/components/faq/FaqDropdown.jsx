...
export default function FaqDropdown({ items, onSelect, onClose }) {
    ...
    return (
        <div className="absolute top-full left-0 w-full bg-white border border-gray-200 rounded-t-md rounded-b-lg shadow-lg z-10 -mt-1 max-h-60 overflow-y-auto">
            <ul>
                ...
                {items.map((faq, index) => (
                    <li key={index} className="px-4" >
                        ...
                        <button
                            ...
                            className="w-full text-left py-3 text-black border-b border-[#002147] "
                        >
                            {faq.question}
                        </button>
                    </li>
